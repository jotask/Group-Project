...
<html lang="en">
    ...
    <?php 
		$dbTaskNum = mysqli_query ($connection, "SELECT * FROM TASK");
		$rowCount = mysqli_num_rows($dbTaskNum);
		if ($rowCount > 0){
...
// Display task data after being sorted
			...
			while ($a = mysqli_fetch_array($dbSort)) {
				...
				echo "<tr>";
				echo '<form action="reallocateAssign.php" method="post">';
				echo '<td><input type="hidden" name="taskID" value="' . $taskID . '"/><input type="submit" value="Reassign" ></td>';
				echo "</form>";
				echo "<td>" . $a[0] . "</td>";
				echo "<td>" . $a[1] . "</td>";
				echo "<td>" . $a[2] . "</td>";
			...
			echo "</table>\n";
// Print out a message if there are no tasks in the database
		}else{
